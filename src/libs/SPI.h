...
class SPIClass {
 public:
  ...
  inline static void setBitOrder(uint8_t bitOrder) {
    if (bitOrder == LSBFIRST) {SPCR |= _BV(DORD); } else {
      SPCR &= ~(_BV(DORD));
    }
  }
  // This function is deprecated.   New applications should use
...
class SPIClass {
 public:
  ...
  static void setBitOrder(uint8_t bitOrder) {
    ...
    if (bitOrder == LSBFIRST) {c |= SPI_C1_LSBFE; } else {c &= ~SPI_C1_LSBFE; }
    SPI0_C1 = c;
  }
  ...
  static void setDataMode(uint8_t dataMode) {
    uint8_t c = SPI0_C1 | SPI_C1_SPE;
    if (dataMode & 0x04) {c |= SPI_C1_CPHA; } else {c &= ~SPI_C1_CPHA; }
    if (dataMode & 0x08) {c |= SPI_C1_CPOL; } else {c &= ~SPI_C1_CPOL; }
    SPI0_C1 = c;
  }
...
class SPISettings {
 ...
 private:
  ...
  __attribute__((__always_inline__)) {
    ...
    config = (dataMode & 3) | SPI_CSR_CSAAT | SPI_CSR_SCBR(div) |
        SPI_CSR_DLYBCT(1);
  }
  uint32_t config;
  BitOrder border;
...
class SPIClass {
 public:
  ...
  byte transfer(uint8_t _data, SPITransferMode _mode = SPI_LAST) {
    return transfer(BOARD_SPI_DEFAULT_SS,
      _data,
      _mode);
  }
  ...
  void setClockDivider(uint8_t _div) {
    setClockDivider(BOARD_SPI_DEFAULT_SS,
        _div);
  }
