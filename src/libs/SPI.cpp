...
void SPIClass::begin() {
  ...
  SPI0_CTAR1 =
      SPI_CTAR_FMSZ(15) | SPI_CTAR_PBR(0) | SPI_CTAR_BR(1) | SPI_CTAR_CSSCK(1);
...
const uint16_t SPISettings::ctar_div_table[23] = {
    ...
    40, 56, 64, 96, 128, 192, 256, 384, 512, 640, 768};
...
bool SPIClass::pinIsChipSelect(uint8_t pin) {
  if (pin == 10 || pin == 9 || pin == 6 || pin == 2 || pin == 15) {
    return true;
  }
  if (pin >= 20 && pin <= 23) {
    return true;
...
bool SPIClass::pinIsChipSelect(uint8_t pin1, uint8_t pin2) {
  if (!pinIsChipSelect(pin1) || !pinIsChipSelect(pin2)) {
    return false;
  }
  if ((pin1 == 2 && pin2 == 10) || (pin1 == 10 && pin2 == 2)) {
    return false;
  }
  if ((pin1 == 6 && pin2 == 9) || (pin1 == 9 && pin2 == 6)) {
    return false;
  }
  if ((pin1 == 20 && pin2 == 23) || (pin1 == 23 && pin2 == 20)) {
    return false;
  }
  if ((pin1 == 21 && pin2 == 22) || (pin1 == 22 && pin2 == 21)) {
    return false;
  }
  return true;
}
...
const uint8_t SPISettings::br_clock_table[30] = {
    SPI_BR_SPPR(0) | SPI_BR_SPR(0), SPI_BR_SPPR(1) | SPI_BR_SPR(0),
    SPI_BR_SPPR(2) | SPI_BR_SPR(0), SPI_BR_SPPR(3) | SPI_BR_SPR(0),
    SPI_BR_SPPR(4) | SPI_BR_SPR(0), SPI_BR_SPPR(5) | SPI_BR_SPR(0),
    SPI_BR_SPPR(6) | SPI_BR_SPR(0), SPI_BR_SPPR(7) | SPI_BR_SPR(0),
    SPI_BR_SPPR(4) | SPI_BR_SPR(1), SPI_BR_SPPR(5) | SPI_BR_SPR(1),
    SPI_BR_SPPR(6) | SPI_BR_SPR(1), SPI_BR_SPPR(7) | SPI_BR_SPR(1),
    SPI_BR_SPPR(4) | SPI_BR_SPR(2), SPI_BR_SPPR(5) | SPI_BR_SPR(2),
    SPI_BR_SPPR(6) | SPI_BR_SPR(2), SPI_BR_SPPR(7) | SPI_BR_SPR(2),
    SPI_BR_SPPR(4) | SPI_BR_SPR(3), SPI_BR_SPPR(5) | SPI_BR_SPR(3),
    SPI_BR_SPPR(6) | SPI_BR_SPR(3), SPI_BR_SPPR(7) | SPI_BR_SPR(3),
    SPI_BR_SPPR(4) | SPI_BR_SPR(4), SPI_BR_SPPR(5) | SPI_BR_SPR(4),
    SPI_BR_SPPR(6) | SPI_BR_SPR(4), SPI_BR_SPPR(7) | SPI_BR_SPR(4),
    SPI_BR_SPPR(4) | SPI_BR_SPR(5), SPI_BR_SPPR(5) | SPI_BR_SPR(5),
    SPI_BR_SPPR(6) | SPI_BR_SPR(5), SPI_BR_SPPR(7) | SPI_BR_SPR(5),
    SPI_BR_SPPR(4) | SPI_BR_SPR(6), SPI_BR_SPPR(5) | SPI_BR_SPR(6)};
...
void SPIClass::begin(uint8_t _pin) {
  ...
  PIO_Configure(g_APinDescription[spiPin].pPort,
...
void SPIClass::setDataMode(uint8_t _pin, uint8_t _mode) {
  ...
  SPI_ConfigureNPCS(spi, ch,
                    mode[ch] | SPI_CSR_SCBR(divider[ch]) | SPI_CSR_DLYBCT(1));
}
